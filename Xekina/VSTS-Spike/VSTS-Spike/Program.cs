...
namespace VSTS_Spike
{
    class Program
    {
        ...
        private static void CreateBuildAndReleaseProcess_old(string projectName)
        {
            Log("Build and Release Process Creation Phase is starting", ConsoleColor.Cyan);
            Log(String.Format("Creating Processes for project {0}", projectName));
            projectHttpClient = GetVssConnection().GetClient<ProjectHttpClient>();
            var projectReferences = projectHttpClient.GetProjects().SyncResult();
            string baseProjectGuidString = projectReferences.Where(pr => pr.Name.Equals(baseProjectName)).FirstOrDefault().Id.ToString();
            string newProjectGuidString = projectReferences.Where(pr => pr.Name.Equals(projectName)).FirstOrDefault().Id.ToString();
            var newProjectReference = projectHttpClient.GetProject(newProjectGuidString).SyncResult();
            
            var buildHttpClient = GetVssConnection().GetClient<BuildHttpClient>();
            var buildDefinitions = buildHttpClient.GetDefinitionsAsync(Guid.Parse(baseProjectGuidString)).SyncResult();
            int baseBuildDefinitionId = buildDefinitions.Where(bd=>bd.Name.Equals(CloudConfigurationManager.GetSetting("BaseBuildProcessName"))).FirstOrDefault().Id;
            var templateBuildDefinition = buildHttpClient.GetDefinitionAsync(baseProjectName, baseBuildDefinitionId).SyncResult();
            //BuildDefinition newBuildDefinition = templateBuildDefinition;
            //newBuildDefinition.Name = projectName + " Build Process";
            //newBuildDefinition.Project = newProjectReference;

            var eliotBuildDefinitions = buildHttpClient.GetDefinitionsAsync(Guid.Parse(newProjectGuidString)).SyncResult();
            int eliotBuildDefinitionId = eliotBuildDefinitions.Where(bd => bd.Name.Equals("eliot-ASP.NET (PREVIEW)-CI")).FirstOrDefault().Id;
            var eliotBuildDefinition = buildHttpClient.GetDefinitionAsync(projectName, eliotBuildDefinitionId).SyncResult();
            BuildDefinition newBuildDefinition = eliotBuildDefinition;
            newBuildDefinition.Name = projectName + " Build Process";
            newBuildDefinition.Project = newProjectReference;
                      
            var temp = JsonConvert.SerializeObject(newBuildDefinition);
            var b = buildHttpClient.CreateDefinitionAsync(newBuildDefinition,
                Guid.Parse(newProjectGuidString)).SyncResult();
            

            Console.ReadLine();

            Log("End of Build and Release Process Creation Phase.", ConsoleColor.Cyan);
            return;
        }
